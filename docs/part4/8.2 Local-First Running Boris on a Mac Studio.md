...
> cloud, no API keys. Focus on:
>
> 1. **Understand now** — why local matters and the four forces driving adoption.
> 2. **Memorize** — hardware sizing rules and the endpoint-swap pattern.
> 3. **Reference later** — the local-vs-cloud router in Concept Loop 5.
>
...
1. A local-vs-cloud decision framework grounded in privacy, latency, and cost.
2. A simulated Ollama-compatible inference stack running on localhost.
3. A hardware-sizing calculator that predicts tokens/sec for any Apple Silicon config.
4. A local agent harness using the endpoint-swap pattern from Section 5.1.
5. A task-routing function implementing local-first, cloud-fallback logic.
...
**Expected output:**
```
HIPAA patient pipeline         → LOCAL  (sensitive data stays on-device, 800,000 tokens cheaper locally)
