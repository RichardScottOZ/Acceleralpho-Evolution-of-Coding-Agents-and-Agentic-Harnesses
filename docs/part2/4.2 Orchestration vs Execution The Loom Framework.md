...
google    = Task("impl_google",     agent="backend",    prompt="Implement Google OAuth flow",
                  depends_on=[migrate])
github_oa = Task("impl_github",     agent="backend",    prompt="Implement GitHub OAuth flow",
                  depends_on=[migrate])
api       = Task("create_api",      agent="backend",    prompt="Create /auth/callback endpoints",
                  depends_on=[google, github_oa])
...
feature_weave.add_tasks([schema, migrate, google, github_oa, api, frontend, tests, docs, e2e])
...
async def run():
    ...
    # Tier 2: google, github_oa (2 agents in parallel)
