...
Notice the trade-off: VS Code has the best *context freshness* (the file watcher updates instantly) but significantly lower throughput, higher latency, and less concurrency than the service-oriented alternatives. This is the monolith trap — it is optimized for one use case (a single human) at the expense of the use case that generates 100× more traffic (agents).
